...
public interface IOrderJpaRepository extends JpaRepository<OrderDbo, String>, JpaSpecificationExecutor<OrderDbo> {
    ...
    @Query("SELECT SUM(o.totalAmount) FROM OrderDbo o WHERE o.status = 'COMPLETED' AND o.orderDate BETWEEN :startDate AND :endDate")
    BigDecimal calculateTotalRevenue(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

    @Query("SELECT COUNT(o) FROM OrderDbo o WHERE o.status = 'COMPLETED' AND o.orderDate BETWEEN :startDate AND :endDate")
    Integer countCompletedOrders(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

    @Query("SELECT COUNT(DISTINCT o.customerId) FROM OrderDbo o WHERE o.status = 'COMPLETED' AND o.orderDate BETWEEN :startDate AND :endDate")
